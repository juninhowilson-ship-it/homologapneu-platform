  ...
  for (const [index, record] of rows.entries()) {
    ...
    try {
      ...
      const drivetrain = record.tracao
        ...
        : null;
      const transmissionType = record.transmissao
        ? normalizeToEnum(record.transmissao, TRANSMISSION_TYPES, TRANSMISSION_LABELS)
        : null;
      ...
      const parsed = veiculoFormSchema.safeParse({
        ...
        wheelbase: record.entreEixos ? Number(record.entreEixos) : null,
        weight: record.peso ? Number(record.peso) : null,
        country: record.pais,
      ...
      if (existing) {
        ...
        const merged: VeiculoFormValues = {
          ...
          transmissionGears:
            parsed.data.transmissionGears ?? current.transmissionGears,
          drivetrain: parsed.data.drivetrain || current.drivetrain || "",
          doors: parsed.data.doors ?? current.doors,
          wheelbase: parsed.data.wheelbase ?? current.wheelbase,
          weight: parsed.data.weight ?? current.weight,
        ...
        const changes = diffRecords(
          ...
          },
          {
            ...
            wheelbase: merged.wheelbase ?? null,
